...
export async function getRestaurantItemModel(restaurantId) {
    ...
    if (restDb.models[collectionName]) {
        // If the compiled model lacks the newly added offerpercentage, oldprices, or offerTitle field, clear it from cache to force recompilation
        if (!restDb.models[collectionName].schema.paths.offerpercentage || !restDb.models[collectionName].schema.paths.oldprices || !restDb.models[collectionName].schema.paths.offerTitle) {
